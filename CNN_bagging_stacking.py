...
def load_all_models(n_models):
    ...
    for i in range(n_models):
        model = tf.keras.models.load_model(f'/HOME/scz0c6m/run/CNN_models_0.8_10/cnn_model_{i}.h5')
        models.append(model)
    return models
...
for images, labels in train_generator:
    ...
    stacked_features = np.hstack(stacked_features)
    stacked_train_data.append(stacked_features)
    stacked_train_labels.extend(labels.numpy())

stacked_train_data = np.vstack(stacked_train_data)
stacked_train_labels = np.array(stacked_train_labels)

meta_model = RandomForestClassifier(n_estimators=100, max_depth=None, random_state=42)

meta_model.fit(stacked_train_data, stacked_train_labels)
...
for images, labels in validation_generator:
    ...
    image_preds = meta_model.predict(stacked_features)
    
    y_true.extend(labels.numpy())
    y_pred.extend(image_preds)
...
print(conf_matrix)
with open("/HOME/scz0c6m/run/CNN_models11/results.txt", "w") as results_file:
